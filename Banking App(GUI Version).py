...
def register():
    ...
    notif.grid(row=6, sticky=N, pady=0)
    ...
    for name in all_accounts:
       if(login_name == name):
           ...
           if(login_password == password):
               ...
               #Buttons
               Button(account_dashboard, text="View Personal Details", font = ("Calibri", 12), width=30, command=personal_details).grid(row=2, sticky=N, padx=10)
...
def deposit():
    ...
    Label(deposit_screen, text="Amount:",font= ("Calibri",12)).grid(row=2, sticky=W, pady=10)
...
def withdraw():
    ...
    current_balance_label.grid(row=1, sticky=W)
    Label(withdraw_screen, text="Amount:",font= ("Calibri",12)).grid(row=2, sticky=W, pady=10)
...
def finish_withdraw():
    ...
    if float(withdraw_amount.get()) <= 0:
        withdraw.config(fg="red", text="Negative currency is not accepted")
        return
...
#image import
img = Image.open("bankingimage.jpg")
img = img.resize((150,150))
img = ImageTk.PhotoImage(img)

#Labels
Label(master, text = "Custom Banking Beta", font = ("Calibri", 14)).grid(row=0, sticky=N, pady=10)
Label(master, text = "The most secure bank you have proabably used.", font = ("Calibri", 12)).grid(row=1, sticky=N)
